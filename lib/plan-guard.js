...
export async function checkPlanLimit(entrepriseId, resource) {
  ...
  if (resource === 'utilisateur') {
    ...
    if (count >= abonnement.max_utilisateurs) {
      throw new PlanLimitError(
        `Limite de ${abonnement.max_utilisateurs} utilisateur(s) atteinte. Passez au plan supérieur.`
      );
    }
  }
  ...
  if (resource === 'chantier') {
    ...
    const count = parseInt(rows[0]?.total ?? 0, 10);
    if (count >= abonnement.max_chantiers) {
      throw new PlanLimitError(
        `Limite de ${abonnement.max_chantiers} chantier(s) actif(s) atteinte. Passez au plan supérieur.`
      );
    }
  }
