...
export function validatePassword(password) {
  ...
  if (password.length < SECURITY_CONFIG.password.minLength) {
    return { isValid: false, error: `Le mot de passe doit contenir au moins ${SECURITY_CONFIG.password.minLength} caractères` };
  }

  if (password.length > SECURITY_CONFIG.password.maxLength) {
    return { isValid: false, error: `Mot de passe trop long (max ${SECURITY_CONFIG.password.maxLength} caractères)` };
  }

  if (SECURITY_CONFIG.password.requireUppercase && !/[A-Z]/.test(password)) {
    return { isValid: false, error: SECURITY_MESSAGES.passwordTooWeak };
  }
  ...
  for (const pattern of SECURITY_CONFIG.password.commonPatterns) {
