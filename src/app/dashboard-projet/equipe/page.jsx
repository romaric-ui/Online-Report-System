...
import {
  ArrowLeft, Users, Mail, UserPlus, Copy, Check,
...
export default function EquipePage() {
  ...
  const fetchMembers = async () => {
    ...
    try {
      ...
      // Pré-sélectionner le premier rôle non-admin si pas déjà sélectionné
      if (!invRole && json.data.roles.length) {
        const defaultRole = json.data.roles.find(r => r.nom !== 'admin');
        if (defaultRole) setInvRole(String(defaultRole.id_role_entreprise));
      }
    } catch (err) {
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Gérez les membres et les invitations</p>
            </div>
          </div>
        </div>
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
              <div className="flex items-center gap-2 mb-5">
                <div className="rounded-lg bg-indigo-100 p-2 text-indigo-600"><UserPlus className="w-4 h-4" /></div>
              ...
              {loadingInvit ? (
                ...
                <ul className="divide-y divide-gray-50">
                  {invitationsEnAttente.map(inv => (
                    <li key={inv.id_invitation} className="flex items-center justify-between gap-3 px-5 py-3">
                            ...
                            <td className="px-4 py-3.5">
                              <div className="relative">
                                <select
                                  ...
                                  disabled={isSelf}
                                  className={`appearance-none rounded-lg border px-3 py-1.5 text-xs font-semibold outline-none transition pr-7 ${
                                    ROLE_CLASSES[m.role_nom] || 'bg-gray-100 text-gray-600 border-gray-200'
                                  } ${isSelf ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer hover:brightness-95'} border-transparent`}
                                >
                                  {roles.map(r => (
