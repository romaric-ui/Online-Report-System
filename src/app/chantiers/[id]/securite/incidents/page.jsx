...
const GRAVITE_OPTIONS = [
  { value: "mineur", label: "Mineur", cls: "text-emerald-700 bg-emerald-50" },
...
export default function IncidentsPage({ params: paramsPromise }) {
  ...
  const fetchIncidents = async () => {
    ...
    try {
      const res = await fetch(
        `/api/chantiers/${id}/securite/incidents?limit=100`,
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok || !json.success)
        throw new Error(json.error?.message || "Erreur de création");
      setForm(EMPTY_FORM);
      setShowForm(false);
      fetchIncidents();
    } catch (err) {
  ...
  const handleStatutChange = async (incidentId, newStatut) => {
    ...
    try {
      const res = await fetch(
        `/api/chantiers/${id}/securite/incidents/${incidentId}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-8">
          <div>
            ...
            <p className="mt-2 text-slate-500">
              Déclaration et suivi des incidents sur le chantier.
            </p>
          </div>
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  Renseignez tous les détails de l'incident.
                </p>
              </div>
            </div>

            {formError && (
              <div className="rounded-3xl bg-red-50 border border-red-200 p-4 text-red-700 mb-4">
                {formError}
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="grid gap-5 sm:grid-cols-2">
                <label className="space-y-2">
                  ...
                  >
                    {TYPE_INCIDENT_OPTIONS.map((o) => (
                      <option key={o.value} value={o.value}>
                        {o.label}
                      </option>
                    ))}
                  </select>
                  {form.type_incident === "autre" && (
                    <label className="block space-y-2 sm:col-span-2">
                      ...
                      <input
                        ...
                        onChange={handleChange("type_autre")}
                        className="w-full rounded-3xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none focus:border-indigo-500"
                  ...
                  <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        <span
                          className={`rounded-full px-3 py-1 text-xs font-semibold ${GRAVITE_CLASSES[inc.gravite] || "bg-slate-100 text-slate-600"}`}
                        >
                      ...
                      {inc.jours_arret > 0 && (
                        <p className="text-sm text-red-600 mt-1 font-medium">
                          {inc.jours_arret} jour(s) d'arrêt
                        </p>
                      )}
                    </div>
                    <div className="flex items-center gap-3 flex-shrink-0">
                      <span
                        className={`rounded-full px-3 py-1 text-xs font-semibold ${STATUT_CLASSES[inc.statut] || "bg-slate-100 text-slate-600"}`}
                      >
                        {STATUT_OPTIONS.find((o) => o.value === inc.statut)
                          ?.label || inc.statut}
                      </span>
                      <select
                        value={inc.statut}
                        ...
                        onChange={(e) =>
                          handleStatutChange(inc.id_incident, e.target.value)
                        }
