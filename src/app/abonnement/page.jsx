...
import { Check, Zap, Building2, Star, AlertTriangle, Crown } from 'lucide-react';
...
export default function AbonnementPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/abonnement')
      .then(r => r.json())
      .then(j => { if (j.success) setAbonnement(j.data); })
  ...
  const essaiTotal  = 7;
  const essaiPct    = Math.max(0, Math.round((essaiJours / essaiTotal) * 100));
  ...
  const handleChoosePlan = (plan) => {
    ...
    alert('Paiement Stripe bientôt disponible. Nous vous contacterons pour finaliser votre abonnement.');
  };
  ...
  return (
    <AppLayout>
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* ── Abonnement actuel ── */}
        {!loading && abonnement && (
          <div className={`bg-white rounded-2xl border shadow-sm p-6 mb-8 ${abonnement.statut === 'expire' || abonnement.statut === 'impaye' ? 'border-red-200' : 'border-gray-100'}`}>

            {(abonnement.statut === 'expire' || abonnement.statut === 'impaye') && (
